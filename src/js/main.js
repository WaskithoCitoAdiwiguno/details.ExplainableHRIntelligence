(function () {
  ...
  var SLIDES = [
    ...
    },
    {
      ...
      alt: { id: 'Grafik SHAP Waterfall: kontribusi tiap fitur terhadap prediksi', en: 'SHAP Waterfall chart: each feature\u2019s contribution to the prediction' },
      title: { id: 'SHAP Waterfall: "Mengapa?"', en: 'SHAP Waterfall: the "Why?"' },
      desc: {
        id: 'Bukan sekadar angka probabilitas: waterfall SHAP menunjukkan perjalanan dari baseline E[f(X)] menuju prediksi akhir f(x). Batang merah (NumCompaniesWorked +0.95, OverTime +0.6) mendorong risiko naik, batang biru (JobSatisfaction −0.56) menahannya, per fitur dan per karyawan.',
        en: 'More than a probability: the SHAP waterfall traces the path from baseline E[f(X)] to the final prediction f(x). Red bars (NumCompaniesWorked +0.95, OverTime +0.6) push risk up, blue bars (JobSatisfaction −0.56) hold it down, feature by feature and employee by employee.'
      }
    },
    {
      ...
      desc: {
        id: 'LLM via Groq menerjemahkan angka SHAP menjadi bahasa HR: ringkasan risiko 90.5% (kategori TINGGI), penjelasan faktor utama secara bisnis (mobilitas tinggi, lembur, work-life balance), dan tiga paket rekomendasi konkret: peninjauan beban kerja, jalur karier, dan inisiatif kesejahteraan.',
  ...
  var i18n = {
    id: {
      ...
      flowSub: 'Dari baris CSV menjadi keputusan HR yang bisa dijelaskan, dalam empat langkah.',
      c1h: 'Unggah Data',
      c1b: 'Unggah CSV hasil cleaning dari Notebook A: kolomnya wajib sama seperti data training, dan 1.470 baris langsung termuat.',
      c2h: 'Pilih Karyawan',
      ...
      expNote: '10 algoritme diadu via RandomizedSearchCV pada dataset IBM HR Attrition (1.470 baris, 35 atribut), lalu divalidasi ulang dengan Repeated Stratified K-Fold. Logistic Regression keluar sebagai pemenang, bukan karena paling rumit, tapi karena paling seimbang menangkap kelas minoritas "resign".',
      ...
      exp3: '<b>Gradient Boosting / Random Forest / LightGBM</b>: akurasi tinggi (0.84+), tetapi recall rendah',
      ...
      exp5: '<b>Kalibrasi &amp; Stabilitas</b>: Platt scaling menurunkan Brier Score 0.1578 → 0.1049 sehingga probabilitasnya layak dipercaya; repeated k-fold memastikan hasilnya stabil',
      xaiTitle: 'Penjelasan & Narasi',
      xai1: '<b>SHAP Waterfall</b>: kontribusi tiap fitur dari baseline E[f(X)] menuju f(x) per karyawan',
      xai2: '<b>Risk tiering</b>: berbasis persentil (Rendah · Sedang · Tinggi), bukan ambang absolut, agar adil pada data tidak seimbang (Pavithran & Vadivel, 2026)',
      xai3: '<b>Narasi LLM</b>: Llama-3.3-70B via Groq merender SHAP menjadi Ringkasan · Faktor Utama · Rekomendasi intervensi yang terkondisi tier',
      xai4: '<b>Guardrail</b>: prediksi & SHAP tetap jalan meski kunci LLM tidak tersedia; narasi hanya gagal sendirinya',
      xai5: '<b>Preskriptif</b>: modul simulasi what-if. Ubah satu faktor (mis. lembur) dan lihat dampaknya terhadap risiko sebelum keputusan diambil',
      ...
      ctaSub: 'Notebook eksperimen, modul simulasi what-if, dan seluruh kode sistem tersedia di repo ini. Dikembangkan sebagai Tugas Akhir di ULBI; jurnalnya sedang diajukan ke JANAPATI.',
      ctaBtn1: 'Buka Repo ↗',
    ...
    en: {
      ...
      lede: 'An explainable human capital analytics framework: a calibrated Logistic Regression model estimates the chance an employee resigns, SHAP reveals the "why" feature by feature, and an LLM (Llama 3.3 via Groq) turns those numbers into tier-matched HR narratives and retention recommendations, complete with a what-if simulation for prescriptive analytics.',
      btnCode: 'Repo & Notebook',
      btnHow: 'How It Works',
      ...
      flowTitle: 'User Flow',
      flowSub: 'From a CSV row to an explainable HR decision, in four steps.',
      c1h: 'Upload Data',
      c1b: 'Upload the cleaned CSV from Notebook A: columns must match the training data, and all 1,470 rows load at once.',
      c2h: 'Pick an Employee',
      c2b: 'Every CSV row appears in the "Pick Employee" dropdown, complete with role, department, age, and years of service.',
      c3h: 'Predict & Explain',
      ...
      expNote: '10 algorithms competed via RandomizedSearchCV on the IBM HR Attrition dataset (1,470 rows, 35 attributes), then re-validated with Repeated Stratified K-Fold. Logistic Regression won, not because it is the fanciest, but because it balances the minority "resign" class best.',
      ...
      exp5: '<b>Calibration & Stability</b>: Platt scaling cut the Brier Score 0.1578 → 0.1049, making the probabilities trustworthy; repeated k-fold confirmed the results are stable',
      ...
      xai2: '<b>Risk tiering</b>: percentile-based (Low · Medium · High) instead of absolute thresholds, fair on imbalanced data (Pavithran & Vadivel, 2026)',
      xai3: '<b>LLM Narrative</b>: Llama-3.3-70B via Groq renders SHAP into Summary · Key Factors · tier-conditioned intervention recommendations',
      xai4: '<b>Guardrail</b>: prediction & SHAP keep working even without an LLM key; only the narrative degrades',
      xai5: '<b>Prescriptive</b>: a what-if simulation module. Tweak one factor (e.g. overtime) and see its risk impact before decisions are made',
      ...
      ctaSub: 'The experiment notebook, what-if simulation module, and all system code live in this repo. Developed as a final-year project (Tugas Akhir) at ULBI; the journal is under review at JANAPATI.',
      ctaBtn1: 'Open Repo ↗',
